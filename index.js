const temperatureRequestURL =
  "https://archive-api.open-meteo.com/v1/archive?latitude=35.994&longitude=-78.8986&start_date=2025-09-01&end_date=2025-09-30&daily=temperature_2m_mean,temperature_2m_max,temperature_2m_min,precipitation_sum,rain_sum,wind_speed_10m_mean,wind_speed_10m_max,weather_code,relative_humidity_2m_mean,relative_humidity_2m_max&timezone=America%2FNew_York&temperature_unit=fahrenheit";
const weatherCodeRequestURL =
  "https://archive-api.open-meteo.com/v1/archive?latitude=35.994&longitude=-78.8986&start_date=2025-09-01&end_date=2025-09-30&daily=weather_code&timezone=America%2FNew_York&temperature_unit=fahrenheit";
...
const weather_code = {
  ...
  10: "Mist",
  11: "Patches (shallow fog or ice fog at the station, whether on land or sea, not deeper than about 2 meters on land or 10 meters at sea)",
...
console.log(weather_code["26"]);
async function fetchWeatherData() {
  try {
    const response = await fetch(temperatureRequestURL);
    if (!response.ok) {
      throw new Error("Network response was not ok");
    }
    const data = await response.json();

    // This loop will log the weather code descriptions in the object for each day's weather code
    for (let i = 0; i < data.daily.weather_code.length; i++) {
      const weatherCodeN = data.daily.weather_code[i];
      console.log(weather_code[weatherCodeN]);
      console.log(data.daily.weather_code[i]);
      0;
    }
...
fetch(temperatureRequestURL)
  ...
  .then((data) => {
    ...
    const maxHumidity = data.daily.relative_humidity_2m_max[0];
    const precipitationSum = data.daily.precipitation_sum[0];
    const rainSum = data.daily.rain_sum[0];
    const windSpeedMean = data.daily.wind_speed_10m_mean[0];
    const windSpeedMax = data.daily.wind_speed_10m_max[0];
    tempDisplay.innerHTML =
      `<h2 id= Temperature>Temperature Data for ${stateName} (September 2025)</h2>
            <li>Max Temperature: ${maxTemp}°F</li>` +
      `<li>Min Temperature: ${minTemp}°F</li>` +
      `<li>Mean Temperature: ${meanTemp}°F</li>` +
      `<li>Total Precipitation: ${precipitationSum} mm</li>` +
      `<li>Total Rainfall: ${rainSum} mm</li>` +
      `<li>Average Wind Speed: ${windSpeedMean} km/h</li>` +
      `<li>Max Wind Speed: ${windSpeedMax} km/h</li>` +
      `<li>Average Humidity: ${avgHumidity}%</li>` +
      `<li>Max Humidity: ${maxHumidity}%</li>`;

    let weatherCount = {};
    data.daily.weather_code.forEach((code) => {
      if (weatherCount[code]) {
        weatherCount[code]++;
      } else {
        weatherCount[code] = 1;
      }
    ...
    fetch(weatherCodeRequestURL)
      ...
      .then((data) => {
        ...
        let weatherDaily =
          "<h2> Weather Display for North Carolina September 2025</h2> <ul>";

        for (let i = 0; i < days.length; i++) {
          const date = days[i];
          const code = data.daily.weather_code[i];
          const description = weather_code[code] || "Unknown weather condition";
          weatherDaily += `<p>${date}: ${description} </p>`;
        }
        let weatherSummary =
          "<h2 id= Weather Summary> Weather Summary for September 2025</h2><ul>";
        for (const code in weatherCount) {
          const description = weather_code[code] || "Unknown weather condition";
          weatherSummary += `<li>${description}: ${weatherCount[code]} days</li>`;
        }

        weatherSummary += "</ul>";

        weatherDisplay.innerHTML = weatherDaily;
        weatherDisplay.innerHTML += weatherSummary;
      })
      .catch((error) => {
        console.error("Error fetching weather summary:", error);
        weatherDisplay.innerHTML = "<p>Failed to load weather summary.</p>";
      });
  })
